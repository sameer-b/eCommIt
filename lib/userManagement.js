...
validateUser = function (name, email, password, confirmPassword, request, response) {
	...
	if(!isValidName(name)) {
		response.render('error', {json : ' Invalid Name! ' } );
		response.end();
		return;
	}

	if(password!==confirmPassword) {
		response.render('error', {json : 'Sorry passwords do not match! ' } );
		response.end();
		return;
	}

	if(!IsEmail(email)) {
		response.render('error', {json : 'Invalid email address! ' } );
		response.end();
		return;
	}
	...
	db.users.find( { email: { $in: [ email ] } }, function (err,user){
		...
		if ( user.length>0 ){
			response.render('message' , { json : ' Sorry email already exists! '});
			response.end();
			return;
		}
		else{
			...
			response.render('message' , { json : ' You have successfully registered! '});
			response.end();
			return;
		}
        ...
        db.users.find( { email: { $in: [ email ] } }, function(err,user){
			if(user[0].password!==passwordHash){
				...
				if(rememberMe === true ) {
					...
					response.render('message', {json : 'You are now logged in! ' } );
					response.end();
				}
